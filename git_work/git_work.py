...
def plot_user_story_board(color_map, epics_to_report, issues_to_report, report_end_date, report_start_date,
                          storypoint_regex, tracking_label_names, user):
    fig, ax = plt.subplots(1, 1, figsize=(8, 8))
    # nodes
    label_color_described = {label: False for label in tracking_label_names}
    label_color_described['closed'] = False
    for position, issue_number in enumerate(user_issue_numbers):
        events = get_events(issues_to_report[issue_number])

        # get intervals when this user was assigned to this issue
        assigned_intervals.append(
            (user in issues_to_report[issue_number].assignees, datetime.datetime.now().timestamp()))
        for event in list(events)[::-1]:
            if (event.event == 'assigned'):
                if event.assignee == user:
                    assigned_intervals.append((True, event.created_at.timestamp()))
            if (event.event == 'unassigned'):
                if event.assignee == user:
                    assigned_intervals.append((False, event.created_at.timestamp()))
        assigned_intervals.append((assigned_intervals[-1][0], issues_to_report[issue_number].created_at.timestamp()))
        assigned_intervals = assigned_intervals[::-1]

        assigned_intervals = np.asarray(assigned_intervals)
        ...
        event_log = {label: [issues_to_report[issue_number].created_at] for label in tracking_label_names}
        for event in events:
        ...
        for label in tracking_label_names:
            event_log = EventLog()
            is_assigned = user in issues_to_report[issue_number].assignees
            has_label = label in [lab.name for lab in issues_to_report[issue_number].labels]
            if is_assigned and has_label:
                if closed:
                    event_log(closed_at)
                else:
                    event_log(issues_to_report[issue_number].updated_at)
            for event in list(events)[::-1]:
                pass
            ...
            for block_idx in range(0, len(event_log[label]), 2):
                ...
                if event_log[label][block_idx + 1] < report_start_date:
                    continue
                if not user_assigned_for_interval(assigned_intervals, event_log[label][block_idx],
                                                  event_log[label][block_idx + 1]):
                    continue
                if closed:
    ...
    ax.set_yticklabels(
        [f"#{id}-{get_story_points(issues_to_report[id], storypoint_regex)}SP" for id in issues_to_report], rotation=0)
    plt.setp(ax.get_xticklabels(), Rotation=30, horizontalalignment='right')
    ax.set_title(
        f"Story board for {', '.join([user.login for user in [user]])}\n({', '.join([lab.name for lab in epics_to_report])})")
    plt.tight_layout()
